...
function starOne(input, verbose=true) {
    let res = 0, range = input[0].split('-');
    for (var psw = parseInt(range[0]); psw <=parseInt(range[1]); psw++) if (criteria(psw)) res++;
...
function starTwo(input, verbose=true) {
    let res = 0, range = input[0].split('-');
    for (var psw = parseInt(range[0]); psw <=parseInt(range[1]); psw++) if (stricterCriteria(psw)) res++;
    if (verbose) console.log(`STAR ONE: ${res}`);
    return res
}
